...
function generateHelpText(): string {
  ...
  const lines = ['browse — persistent headless browser for AI agents', '', 'Commands:'];
...
function spawnClaude(userMessage: string, extensionUrl?: string | null, forTabId?: number | null): void {
  ...
  const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(process.env.HOME || '/tmp', '.browse', 'sidebar-agent-queue.jsonl');
